...
describe('token_expr测试',()=>{
    it('基础token匹配',()=>{
        ...
        let node=token_expr(tool,'number',ExprNumberTree)
        expect(node).toEqual(new ExprNumberTree())
        ...
        node=token_expr(tool,'boolean',ExprBooleanTree)
        expect(node).toEqual(new ExprBooleanTree())
        tool=getStream('map')
        node=token_expr(tool,'map',ExprMapTree)
    ...
    it('不匹配返回null',()=>{
        let tool=getStream('number')
        let node=token_expr(tool,'string',ExprStringTree)
